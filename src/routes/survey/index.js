...
// redux action
import {getServeys,surveyUpdate,surveyDelete,surveyAdd} from '../../actions/AppActions';
...
class Survey extends Component {
	...
	render() {
		...
		if(this.props.get_serveys!=null && this.props.get_serveys.length>0) {
			this.props.get_serveys.map((ro)=>{
				data.push(
					[ro.servey_title,
                     ro.servey_link,
		...
		return (
			<div className="about-wrapper">
				<PageTitleBar title={<IntlMessages id="sidebar.survey" />} match={this.props.match} />
...
const mapStateToProps = ({ reducerApp }) => {
	const {  loading,get_serveys,servey_update } = reducerApp;
	return {  loading,get_serveys,servey_update}
  }
  
  export default connect(mapStateToProps, {
	getServeys,surveyUpdate,surveyDelete,surveyAdd
  })(Survey);
