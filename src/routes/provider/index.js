...
// redux action
import {getContact,addContact,updContact,contactDelete,emailValidate} from '../../actions/AppActions';
  ...
  /**
	...
	deleteUserPermanently() {
		if(this.state.contact_id) {
			this.props.contactDelete(this.state.contact_id);
    ...
    return firstName.length > 0  && lastName.length > 0  && emailAddress.length > 0  && about.length > 0  && designation.length > 0 && phoneNumber.length>0 && password.length>0;
	}
	...
	addNewUser(e) {
		...
		if(firstName && lastName && emailAddress && about && designation && phoneNumber && password && provider_type) {
			...
			if(self.state.upd_id) { 
				self.props.updContact(self.state.upd_id,data)
   ...
   componentWillReceiveProps(next){
	...
	render() {
		const columns = ["Profile Image","Doctor Name", "Designation","Email","Phone Number", "About", "Date & Time","Type", "Action"];
		const data = [];
		const { editUser } = this.state
		if(this.props.get_contact!=null && this.props.get_contact.length>0) {
			
			this.props.get_contact.map((ro)=>{

				data.push(
					[
						<div className="media" >
						{ <img src={ro.profilePic} onError={e => {
		...
		return (
			<div className="about-wrapper">
				<PageTitleBar title={<IntlMessages id="sidebar.provider" />} match={this.props.match} />
        ...
        <FormGroup>
            ...
            <Input
             ...
             />
				...
				<FormGroup>
            <Label for="about">Provider Type <span> * </span></Label>
...
const mapStateToProps = ({ reducerApp }) => {
	const {  loading,get_contact,add_contact,emailCheck } = reducerApp;
	return {  loading,get_contact,add_contact,emailCheck }
  }
  
  export default connect(mapStateToProps, {
	getContact,addContact,updContact,contactDelete,emailValidate
  })(Provider);
